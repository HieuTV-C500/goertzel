...
| `k` | 1×1 | `double`, nguyên, ≥ 0 (`mustBeInteger`, `mustBeNonnegative`) | Chỉ số bin; tần số tương ứng f_k = k·fs/N [Hz] |
...
| `'hop'` | 1×1 | `double` | Tham số tên–giá trị, bước nhảy giữa hai khung (mặc định 205 → không chồng lấp) [mẫu] |
...
*So với Gói #2 (FFT dùng N=256, hop=128, chồng lấp 50%): Goertzel dùng khung N=205, hop=205 (không chồng lấp) vì chỉ cần tính trực tiếp 8 bin cố định (độ phức tạp O(N) mỗi bin), không cần dựng toàn phổ nên không bắt buộc chồng lấp để làm mượt phổ như FFT.*
...
→ Trùng khớp với bảng đã chốt trong `CONTRACTS.md` §"Thông số chốt sẵn". Sai lệch lớn nhất vẫn là 770 Hz (+1,362%), thấp hơn mức 1,461% của Gói #2 (N=256) — do trùng hợp toán học của phép làm tròn `round(f·N/fs)` ở hai giá trị N khác nhau, không phải quy luật chung.
...
**Đối chiếu lý thuyết:** với x[n] = cos(2π·3n/16) là sóng thuần tại đúng bin k=3 của N=16 điểm, biên độ DFT lý thuyết là X[3] = N/2 = 8 ⇒ |X[3]|² = 64 — khớp tuyệt đối với kết quả truy hồi.

**File Excel đi kèm** (`goertzel-N16-k3.xlsx`): sheet `Goertzel_N16_k3` dựng đúng bảng phần B bằng công thức sống (không hardcode) — 2 dòng điều kiện đầu s[-2]=s[-1]=0, 16 dòng n=0..15 kéo công thức `s[n] = x[n] + c*s[n-1] - s[n-2]`, và ô `P` ở cuối; sheet `BangBinK_N205` dựng bảng phần A theo cùng cách.
...
- [ ] Coder cài đặt xong `goertzel_power.m` và đối chiếu với `tests/test_goertzel.m`:
  - `test_knownValue`: N=16, k=3 → P = 64.0 (`AbsTol` 1e-9).
  - `test_matchesBuiltinGoertzel`: N=205, k=18 (~697 Hz), tín hiệu ngẫu nhiên (`rng(1)`) → so với hàm `goertzel()` có sẵn của MATLAB (`RelTol` 1e-10).
  - Việc này ngoài phạm vi Gói #4 (thuộc trách nhiệm Coder); tổ Đặc tả chỉ cần bàn giao đúng công thức + 2 bảng ở trên.
