...
w[n] =
\begin{cases}
0.54 - 0.46\cos\left(\dfrac{2\pi n}{N-1}\right) & 0 \le n \le N-1 \\[4pt]
0 & n \text{ còn lại}
\end{cases}
$$
...
*(N = 256 theo khung FFT của Gói #2; nếu trình đọc của bạn không hỗ trợ hiển thị công thức LaTeX ở trên thì dùng khối chữ bên dưới, nội dung giống hệt.)*
...
**Bin gần nhất với tần số chuẩn f:**

```
k = round(f * N / fs)
```

**% sai lệch giữa bin thực tế và tần số chuẩn:**
...
Dung sai theo `CONTRACTS.md`: **nhận** nếu |sai_lệch| ≤ 1,5% · **từ chối** nếu |sai_lệch| ≥ 3,5%.
